...
(await import("async-exit-hook")).default(() => {
  try {
 ...
 */
const defaultMessageOption: TelegramBot.SendMessageOptions = Object.freeze({
 ...
 */
function onText(regexp: RegExp, cb: (msg: TelegramBot.Message, match: RegExpExecArray | null) => void) {
  /**
   * @param {TelegramBot.Message} msg
   * @param {RegExpExecArray | null} match
   */
  function wrappedCallback(msg: TelegramBot.Message, match: RegExpExecArray | null) {
    if (!msg.text) return;
...
const messageQueue: {
  ...
  throttle: boolean;
  timestamp: number;
  resolve: (value: TelegramBot.Message) => void;
 ...
 */
function sendMessageWithRateLimit(
  chatId: number | string,
  text: string,
  throttle: boolean,
  _options: TelegramBot.SendMessageOptions,
  ...
  bot
    ...
    .catch((err) => {
      ...
      if (inspect(err).includes("429 Too Many Requests")) {
        const sleepTime =
          parseInt(inspect(err).match(/ETELEGRAM: 429 Too Many Requests: retry after (\d+)/)[1], 10) * 1000 || 3000;
        verb(sendMessage, "waiting for", sleepTime, "ms before retrying...");
        sleep(sleepTime).then(() =>
          bot
            .sendMessage(chatId, text, Object.assign(Object.assign({}, defaultMessageOption), _options))
            .then(resolve)
            .catch(reject)
        );
      } else {
...
function markChatThrottledMsgAsSendable(chatId: number) {
...
async function doSendThrottleMessage() {
  ...
  /** @type {string | number} */
  let currChatId: string | number;
  /** @type {number[]} */
  const currChatMsgIndex: number[] = [];
  ...
  const mergedMessages: {
    ...
    options: TelegramBot.SendMessageOptions;
    _options: TelegramBot.SendMessageOptions;
    resolveCallbacks: ((value: TelegramBot.Message) => void)[];
  ...
  for (let i = 0; i < currChatMsgIndex.length; i++) {
    ...
    if (mergedMessages.length === 0) {
      ...
      if (
        equal(currMessage.options, mergedMessages[mergedMessages.length - 1].options) &&
  ...
  for (let i = 0; i < mergedMessages.length; i++) {
    ...
    function resolveAll(mergedMsg: TelegramBot.Message) {
      verb(resolveAll, "resolving all callbacks...");
      mergedMessage.resolveCallbacks.forEach((resolve) => resolve(mergedMsg));
    }
    function rejectAll() {
      verb(rejectAll, "rejecting all callbacks...");
...
async function sendMessage(
   ...
   * @type {TelegramBot.SendMessageOptions}
   */
  const _options: TelegramBot.SendMessageOptions = {
  ...
  if (text.length > 4000) {
    ...
    await sendMessage(chatId, thatText, options);
    return await sendMessage(chatId, thisText, options);
  }
...
async function editMessage(chatId: number | string, msgId: string | number, newText: string) {
  try {
    ...
    return await sendMessage(chatId, newText, defaultMessageOption);
  }
}

async function deleteAfter(chatId: string | number, msgId: number, ms: number) {
  await sleep(ms);
...
async function replyAndDeleteAfter(
...
// 认领包
async function addPackage(chatId: number, msg: TelegramBot.Message, newPackageName: string) {
  ...
  if (packageStatus.some((user) => user.userid === msg.from.id)) {
    packageStatus
      .find((user) => user.userid === msg.from.id)
      .packages.push({
        name: newPackageName,
        lastActive: Date.now(),
      });
...
onText(/^\/rob\s+(\S+)$/, async (msg, match) => {
   ...
   * @param {string} [reason]
   */
  async function mergeCallback(success: boolean, reason: string) {
    if (success) {
  ...
  if (packageStatus.filter((user) => user.packages.some((existingPkg) => existingPkg.name === newPackageName)).length) {
    if (findUserIdByPackage(newPackageName) === msg.from.id) {
      ...
      await replyMessage(
        chatId,
        msg.from.id,
        toSafeMd(i18n`认领失败，该包已被其他人认领，此时你无法通过 /give 转交该包`)
      );
      return;
    }
  } else {
...
onText(/^\/(merge|drop)\s+(\S+)$/, async (msg, match) => {
  ...
  if (match[1] === "merge") {
    await replyMessage(
      chatId,
      msgId,
      "/merge has been deprecated. You should wait for CI/CD to trigger an auto-merge."
    );
  }
 ...
 */
function _merge(mergedPackageName: string, userId: number, callback: (success: boolean, reason?: string) => any) {
  verb("trying to merge", mergedPackageName);

  if (
    !packageStatus.filter((user) => user.packages.some((existingPkg) => existingPkg.name === mergedPackageName)).length
  ) {
  ...
  if (packageStatus.some((user) => user.userid === userId)) {
    const targetPackage = packageStatus
      .find((user) => user.userid === userId)
      .packages.find((pkg) => pkg.name === mergedPackageName);
    if (!targetPackage) {
    ...
    storePackageStatus()
      .then(() => callback(true))
      .catch((err) => callback(false, String(err)));
    return;
  }
...
onText(/^\/_html\s+([\S\s\n]+)$/, (msg, match) => {
  ...
  bot.sendMessage(chatId, html, { parse_mode: "HTML", disable_notification: true }).catch((err) => {
    ...
    sendMessage(chatId, `<pre><code>${safeErr}</code></pre>"`, { parse_mode: "HTML" });
  });
});
...
async function _mark(
  ...
  userId: number,
  mentionLink: string,
  callback: (success: boolean, reason?: string) => any
) {
...
async function markCommandHandler(
  pkg: string,
  mark: string,
  ...
  if (!markConfig) {
    return await replyMessage(
      chatId,
      msgId,
      toSafeMd(`mark ${mark} in getAvailableMarks() but not in getMarkConfig()`)
    );
  }
  ...
  if (markConfig.triggers.length > 0) {
    ...
    for (const trigger of markConfig.triggers) {
      ...
      if (trigger.op === "mark" && !currMarks.includes(trigger.name)) {
        ...
        shouldUnmarkMarks.push(trigger.name);
      }
    }
    ...
    if (shouldUnmarkMarks.length > 0) {
      verb(`triggered by this mark: should also unmark`, shouldUnmarkMarks);
      const prefix = wrapCode("(auto-unmark)");
      await sendMessage(chatId, prefix + toSafeMd(i18n` ${pkg} 将被清除这些标记：${shouldUnmarkMarks.join(" ")}`), {
        parse_mode: "MarkdownV2",
      });
      // we don't care whether triggered unmarks are updated successfully or not,
      // so use a dummy callback
      await _unmarkMultiple(pkg, shouldUnmarkMarks, () => {});
    }
  }

  function markCallback(success: boolean, reason: string) {
    if (success) {
      replyMessage(chatId, msgId, toSafeMd(i18n`状态更新成功`));
      if (String(chatId) !== process.env["PLCT_CHAT_ID"]) {
        sendMessage(
          process.env["PLCT_CHAT_ID"],
          toSafeMd(i18n`${pkg} 已被标记为 ${mark}：${comment || i18n`无注释`}`),
          {
            parse_mode: "MarkdownV2",
          }
        );
...
// Note that `[^[,\s]` is \S except [ and ,
// e.g.                    ^ /markall       pkg0001   , ...,  pkg000n         { failing }    comment
const MARK_MULTI_REGEXP = /^\/markall ((?: *[^[,\s]+ *,)*(?: +[^[,\s]+))\s+(?:{([\s\S]+)})( +[\s\S]+)?$/;

onText(MARK_MULTI_REGEXP, async (msg, match) => {
  const chatId = msg.chat.id;
  const msgId = msg.message_id;
  const userId = msg.from.id;

  const pkgList = match[1].split(",").map((pkg) => pkg.trim());
...
onText(/^\/unmark\s+(\S+)\s+(\S+)/, async (msg, match) => {
  ...
  if (mark === "all") {
    ...
    if (succ) {
      let respText = toSafeMd(i18n`已成功删除该包的 ${targetMarks.map((mark) => wrapCode(mark)).join(" ")} 标记`);
  ...
  async function unmarkCallback(success: boolean, reason?: string) {
    if (success) {
...
async function _unmarkMultiple(pkg: string, marks: string[], callback: (success: boolean, reason?: string) => any) {
  let allSuccessful = true;
 ...
 */
async function _unmark(pkg: string, mark: string, callback: (success: boolean, reason?: string) => any) {
  if (packageMarks.filter((obj) => obj.name === pkg).length > 0) {
    const target = packageMarks.filter((obj) => obj.name === pkg)[0];
    if (target.marks.some((markObj) => markObj.name === mark)) {
...
async function showMarkHelp(chatId: number) {
  await sendMessage(
    chatId,
    toSafeMd(
      i18n`/mark 用法：\n/mark pkg status Optional<comment>\n\n可用的 status 包括 ${Object.keys(
        localUtils.MARK_TO_DESC
      ).join(", ")}`
    ),
    { parse_mode: "MarkdownV2" }
  );
...
async function showMarkAllHelp(chatId: number) {
  await sendMessage(
    chatId,
    toSafeMd(
...
async function routePkgHandler(req: http.IncomingMessage, res: http.ServerResponse) {
  ...
  if (!url.searchParams.has("mark")) {
    ...
    const packages = packageMarks
      .filter((pkg) => pkg.marks.some((mark) => mark.name === markName))
      .map((pkg) => pkg.name);
    res.writeHead(200, { "Content-Type": "application/json" });
...
async function routeDeleteHandler(req: http.IncomingMessage, res: http.ServerResponse) {
  const url = new URL(req.url, `http://${req.headers.host}`);
  ...
  const pkgname = decodeURIComponent(args[1]),
    status = args[2];
  ...
  if (userId === null) {
    ...
    sendMessage(
      CHAT_ID,
      msgTypeStr + " ping " + link + toSafeMd(i18n`: ${pkgname} 已出包`),
      {
        parse_mode: "MarkdownV2",
      },
      true
    ...
    _merge(pkgname, userId, (success, reason) => {
      ...
      sendMessage(
        CHAT_ID,
        msgTypeStr + toSafeMd(` failed: ${reason}`),
        {
          parse_mode: "MarkdownV2",
        },
        true
  ...
  (async function fencedAtomicOps() {
    ...
    await _unmarkMultiple(pkgname, targetMarks, (success, reason) => {
      ...
      sendMessage(
        CHAT_ID,
        msgTypeStr + toSafeMd(i18n` ${pkgname} 已出包，不再被标记为 ${mark}`),
        {
          parse_mode: "MarkdownV2",
        },
        true
    ...
    for (const pkg of targetPackages) {
      ...
      const mentionLinkSet: Set<string> = new Set();
        ...
        if (mark.comment.toLowerCase() === `[${pkgname}]`.toLowerCase()) {
          ...
          await _unmark(pkg.name, mark.name, (success, _) => {
            ...
            defer.add(deferKey, () => {
              sendMessage(
                CHAT_ID,
                msgTypeStr + toSafeMd(i18n` ${pkg.name} 因 ${pkgname} 出包，不再被标记为 ${mark.name}`),
                {
                  parse_mode: "MarkdownV2",
                },
                true
          ...
          await _mark(pkg.name, mark.name, comment, uid, mentionLink, (success, _) => {
            ...
            defer.add(deferKey, () => {
              sendMessage(
                CHAT_ID,
                msgTypeStr + toSafeMd(i18n` [${pkgname}] 已从 ${pkg.name} 的 ${mark.name} 状态内移除。`),
                {
                  parse_mode: "MarkdownV2",
                },
                true
...
async function routeAddHandler(req: http.IncomingMessage, res: http.ServerResponse) {
  const url = new URL(req.url, `http://${req.headers.host}`);
  const args = url.pathname.slice(1).split("/");
  ...
  if (userId === null) {
    ...
    sendMessage(
      CHAT_ID,
      _safemd`${msgTypeStr} ping ${link}: ${toSafeMd(pkgname)} ${failingLogLink}`,
      {
        parse_mode: "MarkdownV2",
      },
      true
  ...
  await _unmarkMultiple(pkgname, ["ready"], (success, reason) => {
    ...
    defer.add(deferKey, () => {
      sendMessage(
        CHAT_ID,
        msgTypeStr + toSafeMd(i18n` ${pkgname} 不再被标记为 ${mark}`),
        {
          parse_mode: "MarkdownV2",
        },
        true
